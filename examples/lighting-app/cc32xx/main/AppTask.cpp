...
int AppTask::Init()
{
    ...
    // Initialize Lighting App module
    PLAT_LOG("Initialize Lighting App");
    LightMgr().Init();
...
void AppTask::ActionInitiated(LightingManager::Action_t aAction, int32_t aActor)
{
    ...
    {
        PLAT_LOG("On initiated");
        ; // TODO
    }
    else if (aAction == LightingManager::OFF_ACTION)
    {
        PLAT_LOG("Off initiated");
        ; // TODO
    }
...
void AppTask::DispatchEvent(AppEvent * aEvent)
{
    ...
    case AppEvent::kEventType_IdentifyStart:
        ...
        case IDENTIFY_TRIGGER_EFFECT_BLINK:
            LED_setOn(gLedGreenHandle, LED_BRIGHTNESS_MAX);
            LED_startBlinking(gLedGreenHandle, 1000, LED_BLINK_FOREVER);
            break;
        case IDENTIFY_TRIGGER_EFFECT_BREATHE:
            LED_setOn(gLedGreenHandle, LED_BRIGHTNESS_MAX);
            LED_startBlinking(gLedGreenHandle, 100, LED_BLINK_FOREVER);
            break;
        case IDENTIFY_TRIGGER_EFFECT_OKAY:
            LED_setOn(gLedGreenHandle, LED_BRIGHTNESS_MAX);
            LED_startBlinking(gLedGreenHandle, 500, LED_BLINK_FOREVER);
            break;
        default:
            break;
        }
